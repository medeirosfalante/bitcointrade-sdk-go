...
func request(method string, command *string, payloadJSON []byte, queryStringByte []byte) (*Response, error) {
	uri := fmt.Sprintf("%s/%s", GetBasePath(), *command)
	log.Printf(uri)
...
func (web WebClient) GET(command string, queryString []byte) (*Response, error) {
	response, err := request("GET", &command, nil, queryString)
	return response, err
}

// POST func in WebClient
func (web WebClient) POST(command string, payload []byte) (*Response, error) {
	response, err := request("POST", &command, payload, nil)
	return response, err
}
